...
Older versions of Windows and some Linux installers will sometimes overwrite the entire UEFI partition, but this has been a known problem for several years. You’ll want to mount an existing Windows UEFI partition without formatting it to /boot/efi on your Linux system. So, always use a shared UEFI partition, even if you’re installing Linux to a secondary drive as it will give you an easier time with Secure Boot, BitLocker, and GRUB2.
