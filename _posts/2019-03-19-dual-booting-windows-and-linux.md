...
Older versions of Windows and some Linux installers will sometimes overwrite the entire UEFI partition, but this has been a known problem for several years. You want to mount your existing Windows UEFI partition without formatting it to /boot/efi on the Linux system. **Always use a shared UEFI partition**, even if installing Linux to a secondary drive as this will give you an easier time when dealing with Secure Boot, BitLocker, and GRUB2.
