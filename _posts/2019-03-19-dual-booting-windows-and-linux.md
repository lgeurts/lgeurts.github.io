...
The OS-prober should auto-detect Windows and create a boot menu item for it alongside Linux in GRUB2. Because Windows Update requires multiple reboots, you must configure GRUB bootloader to remember the most recent boot menu (GRUB_DEFAULT=saved; GRUB_SAVEDEFAULT=true). This will allow both the operating systems to trigger multiple reboots when performing updates and have it boot into the correct operating system. It’ll also get you back into the same operating system that you used the last time you booted your system.
