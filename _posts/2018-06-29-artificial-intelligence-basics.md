...
As we started with data that includes the label we try to predict, we can train the model using only some data. And withhold the rest for evaluating the models performance. Then we can use the model to predict (F) of (X) for evaluation data and compare the predictions or scored labels to the actual labels that we know to be true.
