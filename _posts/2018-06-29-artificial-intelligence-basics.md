...
A sample of one person isn't likely to give a function that generalizes well. So we gather the same data from a large number participants, and then train the model using the bigger set of data. After training, and now having a new generalized function that can be used to calculate our label (Y), we can plot the values of (Y), calculated for specific features of (X) values.
