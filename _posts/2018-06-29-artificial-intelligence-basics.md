...
At heart, a function is the mapping from a set in a ***domain*** to a set in a ***codomain***. A function can map a set to itself. For example, *`f(x) = x2`*, also notated *`f: x ↦ x2`*, is the mapping of all real numbers to all real numbers, or *`f: R → R`*. The ***range*** is the subset of the codomain which the function maps to (a function doesn't necessarily map to every value in the codomain. Where it does, the range equals the codomain).
