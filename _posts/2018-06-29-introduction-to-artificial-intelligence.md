...
In a ***unsupervised*** learning scenario, we don't have known label values in our training data set. Instead We train the model by finding similarities between observations. Once we trained the model every new observation is added to a group (cluster) of observations with similar characteristics.
