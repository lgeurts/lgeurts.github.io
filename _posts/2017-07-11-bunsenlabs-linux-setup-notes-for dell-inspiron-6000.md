...
In rare cases when using a WiFi connection to connect to the internet, the Pro /2200BG adapter is not recognized during setup. Just ignore when it wants to download system updates. Same goes for anything related to Bunsen repositories. After install WiFi will work fine, and you can add [the repos](https://forums.bunsenlabs.org/viewtopic.php?id=1526) in /etc/apt/sources.list.
