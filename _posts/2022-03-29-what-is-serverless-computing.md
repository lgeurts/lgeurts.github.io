...
Speaking about cost efficiency, in contrast with VMs and containers, serverless services don't run continuously. A serverless configuration will only run when it's invoked and will afterwards release all computing capacity. This is the reason why a serverless architecture is the most cost-effective way since you won’t have to pay for idle time.
