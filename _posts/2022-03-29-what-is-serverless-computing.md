...
Since a VM has its own OS, it also has its own kernel which provides for a secure boundary over other VMs running in our host computer. It takes time to launch all of these instances due to the different components that a hypervisor must virtualize and allocate. Moreover, you will have to pay the cost of running your VMs, including idling time where a resource isn’t used at all. And let's not forget that each of these virtual machines is running a guest operating system that needs regular patching. All in all this concept has some disadvantages.
