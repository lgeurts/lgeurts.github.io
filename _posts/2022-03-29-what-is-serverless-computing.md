...
Back in the days, a company had to buy expensive physical servers, set them up on-site in rooms that were usually outfitted with peripheral subsystems for cooling, ventilation, fire suppression, etc., and configure each server in order to run their applications. They also has to hire a team of systems and network administrators to maintain these bare-metal servers and troubleshoot any issues that could occur along the way.
